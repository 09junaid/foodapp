    ...
    <section className="py-12 px-6 bg-[#F8F7F2]">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center flex-col justify-center gap-4 mb-12">
          <p className="text-lg text-[#8EC63F] font-medium">Free and Fast</p>
          <h1 className="text-5xl md:text-6xl font-bold mt-2 mb-4">Order Now</h1>
        ...
        <form
          ...
          className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-white p-10 rounded-xl shadow-xl"
        >
          {inputFields.map((item, index) => (
            <div key={index} className="flex flex-col">
              <label className="mb-2 font-semibold text-[20px] text-gray-700">
                {item.label}
              </label>
              ...
              {item.type === "textarea" ? (
                <textarea
                  ...
                  className="p-4 shadow-md bg-[#F8F7F2] rounded-lg resize-none text-base"
                  rows={6}
                />
              ) : (
                <input
                  ...
                  placeholder={item.placeholder}
                  className="p-4 shadow-md bg-[#F8F7F2] rounded-lg text-base"
                />
              )}

              {errors[item.name as keyof OrderPayload] && (
                <span className="text-red-500 text-sm mt-1">
                  {(errors[item.name as keyof OrderPayload] as any).message ||
          ...
          <div className="md:col-span-2 text-center mt-6">
            <button
              type="submit"
              className="bg-[#8EC63F] hover:bg-[#8EC63F]/80 text-white px-8 py-3 rounded-full text-[16px] cursor-pointer font-semibold transition"
              disabled={loading}
            >
